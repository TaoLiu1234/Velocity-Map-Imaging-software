...
def parse_out_file(filename: str) -> MainData:
    ...
    for fg, lens_VMI, block_data in all_blocks:
        ...
        for ion_n, traj in ion_trajectories.items():
            ...
            temp = data[fg][lens_VMI]['local'][p_idx]
            if not isinstance(temp, dict):
                data[fg][lens_VMI]['local'][p_idx] = {'trajectories': temp[:] if hasattr(temp, '__len__') else []}
            data[fg][lens_VMI]['local'][p_idx]['trajectories'].append(traj)
...
def calculate_detector_stats(data: MainData, x_range=(73.0, 166.0)):
    ...
    def compute_detector_stats_for_fg(fg, fg_data):
        ...
        for lens_VMI in sorted(fg_data.keys()):
            ...
            global_data_per_lens[lens_VMI] = global_data
        return fg, global_data_per_lens
...
def process_data(x_range: Tuple[float, float], file_path: str, focus_axis: str, fly2_file: str = None, y_range: Tuple[float, float] = (-0.5, 0.5), z_range: Tuple[float, float] = (-0.5, 0.5)):
    ...
    try:
        ...
        for fg in data:
            for lvmi in data[fg]:
                global_data = data[fg][lvmi]['global']
...
def compute_for_fg_lens(fg, lens_VMI, lens_data, electron_energy=5):
    ...
    # Calculate wavelength for de Broglie relation
    from scipy import constants
    m_e = constants.m_e
    E_j = electron_energy * constants.e
    p = np.sqrt(2 * m_e * E_j)
    lambda_e = constants.h / p  # in meters
    lambda_e_mm = lambda_e * 1000  # in mm
    k = np.pi / (4 * lambda_e_mm)  # Rayleigh range scaling factor
    rayleigh_ranges_y = k * focus_widths_y ** 2
    rayleigh_ranges_z = k * focus_widths_z ** 2
...
def data_viewer(data, mode='single', focus_axis='y', fg_idx=None, fly2_file=None, r2_threshold=0.2, num_groups=5, y_range=(-0.5, 0.5), z_range=(-0.5, 0.5), x_range=(73.0, 166.0), electron_energy=5):
    ...
    if mode == 'single':
        ...
        all_data = {}
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(compute_for_fg_lens, fg, lens, data[fg][lens], electron_energy): (fg, lens) for fg in fg_keys for lens in data[fg].keys()}
            for future in as_completed(futures):
...
def para_2d_landscape(data, target='peak_y', fly2_file=None):
    ...
    for (fg, lens), mets in metrics.items():
        ...
        if not (np.isnan(val) or np.isinf(val)):  # Only include finite values
            ...
            values.append(val)
    if not points:
        print(f"No valid data points for target '{target}'. Cannot create landscape plot.")
        plt.close(fig)
        return
    ...
    if len(np.unique(points[:, 0])) < 2 or len(np.unique(points[:, 1])) < 2:
        ...
        ax.set_title(f'Parameter Scatter: {target_labels[target]}')
        plt.show()
        return
    # Interpolate
...
def extract_aligned_points_for_all_pairs(data, x_range=(73.0, 166.0), x_step=1):
    ...
    for fg in data:
        for lvmi in data[fg]:
            ...
            # Store in global
            data[fg][lvmi]['global']['focus_points_y'] = focus_points_y
            data[fg][lvmi]['global']['focus_points_z'] = focus_points_z
