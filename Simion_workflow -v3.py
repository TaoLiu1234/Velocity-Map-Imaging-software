...
NUM_POINTS = 30   # Number of field gradient points to simulate (reduced from v2's 20 for faster testing)
LENS_MIN = 1.25    # Minimum lens focusing factor (lens_VMI), adjusted from v2's 1.38
...
NUM_GROUPS = 10  # Number of particle groups to generate (each group has 8 trajectories, reduced from v2's 10)
X_SCAN_RANGE = (73.0, 165.0)  # Range of x-planes to scan for focus analysis (in mm)
SCAN_STEP = 0.25  # Step size for x-plane scanning (in mm)
...
param = Utilis.compute_vmi_parameters(
    ...
    save_to_file=False, filename='parameters.mat'  # Don't save to file, keep in memory
)
...
Utilis.generate_particles_fly2(num_groups=NUM_GROUPS, filename=OUTPUT_FILENAME_FLY2, x_range=(-0.5, 0.5), y_range=(-0.5, 0.5), z_range=(-0.5, 0.5))
...
processed_data = Utilis.process_data(
    ...
    x_step=SCAN_STEP,      # Step size for scanning
...
Utilis.data_viewer(processed_data, mode='multiple', focus_axis=FOCUS_CRITERION, fly2_file=OUTPUT_FILENAME_FLY2, r2_threshold=0.2, num_groups=NUM_GROUPS)
