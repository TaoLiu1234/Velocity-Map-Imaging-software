...
export const CodeNode = React.memo<NodeProps<CanvasNodeData>>(function CodeNode(props) {
  const { id, data } = props
  const [isEditing, setIsEditing] = useState(false)
  const [draft, setDraft] = useState(data.content ?? '// code')
  ...
  const handlePreDoubleClick = useCallback(() => {
    startEdit()
  }, [startEdit])

  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      <Handle type="target" position={Position.Left} id="left-target" style={{ opacity: 0.02, width: 28, height: 28, top: '50%' }} />
      <Handle type="source" position={Position.Left} id="left-source" style={{ top: '50%', width: 14, height: 14 }} />

      <BaseCard title={data.title ?? 'Code'} color={data.color} onTitleChange={commitTitle} selected={props.selected}>
        {isEditing ? (
          <textarea
            value={draft}
            autoFocus
          ...
          <pre
            ...
            onDoubleClick={handlePreDoubleClick}
          >
            {data.content ?? '// 双击编辑代码'}
          </pre>
        )}
      ...
      <Handle type="target" position={Position.Right} id="right-target" style={{ opacity: 0.02, width: 28, height: 28, top: '50%' }} />
