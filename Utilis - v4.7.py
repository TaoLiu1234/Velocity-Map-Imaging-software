        ...
        for line_num, line in enumerate(f, 1):
            ...
            if len(parts) == 4:
                try:
                    ...
                    ion_n, x, y, z = int(parts[0]), np.float16(parts[1]), np.float16(parts[2]), np.float16(parts[3])
...
def process_simulation_block(data, fg, lens_VMI, ke, block_data):
    ...
    for ion_n, traj in ion_trajectories.items():
        ...
        temp = data[fg][lens_VMI][ke]['local'][p_idx]
        if not isinstance(temp, dict):
            data[fg][lens_VMI][ke]['local'][p_idx] = {'trajectories': temp[:] if hasattr(temp, '__len__') else []}
...
def rotate_around_z(vec, theta):
    """
    Rotate a 3D vector around the z-axis by theta radians.
    ...
    # Normalize
    length = math.sqrt(new_x**2 + new_y**2 + z**2)
    if length == 0:
        return (0, 0, 0)
    return (new_x / length, new_y / length, z / length)
...
def generate_particles_fly2(num_groups=4, filename='particles_tao.fly2', x_range=(-0.5, 0.5), y_range=(-0.5, 0.5), z_range=(-0.5, 0.5), ke=15, theta=0):
    ...
    directions = [rotate_around_z(d, theta) for d in original_directions]
...
def data_viewer(data, mode='single', focus_axis='y', fg_idx=None, fly2_file=None, r2_threshold=0.2, num_groups=5, y_range=(-0.5, 0.5), z_range=(-0.5, 0.5), x_range=(73.0, 166.0), electron_energy=5):
    """
    ...
    if mode == 'single':
        ...
        all_data = {}
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(compute_for_fg_lens, fg, lens, data[fg][lens][ke], electron_energy): (fg, lens, ke) for fg in fg_keys for lens in data[fg].keys() for ke in data[fg][lens].keys()}
            for future in as_completed(futures):
        ...
        ax_info.text(0, 0, "", transform=ax_info.transAxes, fontsize=8, verticalalignment='top', fontfamily='monospace')
        ...
        def update_lens_slider(fg_idx, current_lens=None):
            ...
            if current_lens is not None and current_lens in lens_keys:
                ...
                lens_slider.val = 0
            lens_slider.valtext.set_text(f'{lens_keys[int(lens_slider.val)]}')
        ...
        def on_fg_change(val):
            current_fg = fg_keys[int(fg_slider.val)]
            current_lens_keys = sorted(data[current_fg].keys())
            current_lens = current_lens_keys[int(lens_slider.val)]
            update_lens_slider(int(val), current_lens)
            update()
...
def aberration_estimation(data, theta=0):
    """
    Estimate aberrations for each field gradient (fg), lens_VMI, and kinetic energy (ke).

    Aberrations computed:
    - Field curvature: Petzval slope for XZ and XY planes (slope_y and slope_z)
    - Astigmatism: np.abs((std_x_y - std_x_z) / ((std_x_y + std_x_z) / 2)) where std_x_y/z are std of x-focus positions
    - Spherical aberration: Fit r = a + b * theta**3 where theta is emission elevation angle (in degrees), r is radial focus position
    - Chromatic aberrations (computed per fg, lens_VMI):
        - Longitudinal: ptp of intercepts from field curvature fits over ke
        - Lateral: ptp of x and y positions from focus points over ke

    Args:
        data: Processed SIMION data dictionary
        theta: Rotation angle (in radians) around z-axis for emission angles

    Returns:
        dict: Aberration results for each fg, lens_VMI, ke (including chromatic per fg, lens_VMI)
    """
        ...
        for lens_VMI in sorted(data[fg].keys()):
            ...
            for ke in sorted(data[fg][lens_VMI].keys()):
                    ...
                    for group_idx in range(1, 6):  # Groups 1-5
                        if group_idx in group_angles:
                            ...
                            if group_idx - 1 < len(focus_points_y):
                                ...
                                r_values.append(r)
                                theta_values.append(group_angles[group_idx]**3)  # theta**3
                ...
                results[fg][lens_VMI][ke] = {
                    ...
                    'chromatic_longitudinal': chromatic_per_fg_lens[fg][lens_VMI]['longitudinal_chromatic_xz'] if chromatic_per_fg_lens[fg][lens_VMI]['longitudinal_chromatic_xz'] != float('inf') else chromatic_per_fg_lens[fg][lens_VMI]['longitudinal_chromatic_xy'],
                    'chromatic_lateral': {
                        'y_focus': chromatic_per_fg_lens[fg][lens_VMI]['lateral_chromatic_y_focus'],
                        'z_focus': chromatic_per_fg_lens[fg][lens_VMI]['lateral_chromatic_z_focus']
                    }
                }
...
def print_aberration_summary(aberration_results):
    """
    Print a summary of aberration results.

    Args:
        aberration_results: Output from aberration_estimation function
    """
...
def format_value(val):
    """Format numerical values, handling inf/nan."""
    if val is None or np.isinf(val) or np.isnan(val):
        return 'N/A'
    return f'{val:.6f}'
