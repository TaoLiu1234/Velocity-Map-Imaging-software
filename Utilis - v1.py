...
FieldGradientData = Dict[str, ParticleData]
MainData = Dict[float, Dict[str, Any]]
AnalysisResults = Dict[float, Dict[int, Dict[int, Dict[int, List[float]]]]]
...
def parse_out_file(filename: str) -> MainData:
    ...
    Returns:
        ...
        `data[field_gradient]['local'][particle_idx] = [trajectory_1, ...] `.
    """
    param_pattern = re.compile(r'parameters\s*=\s*\[([^\]]+)\]')
    data: MainData = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    
    all_blocks: List[Tuple[float, List[Tuple[int, float, float, float]]]] = []
    ...
    with open(filename, 'r') as f:
        for line_num, line in enumerate(f, 1):
            if "Begin Next Fly'm" in line:
                if current_fg is not None and current_block_data:
                    all_blocks.append((current_fg, current_block_data))
                current_block_data = []
                current_fg = None
                continue
    ...
    for fg, block_data in all_blocks:
        ...
        for ion_n, traj in ion_trajectories.items():
            ...
            data[fg]['local'][p_idx].append(traj)
...
def calculate_final_position_stats(data: MainData):
    ...
    for fg in sorted(data.keys()):
        ...
        for p_idx in sorted(particles.keys()):
            ...
            final_positions = np.array([traj[-1][1:3] for traj in trajs])
            
            for idx in np.int32(np.linspace(0,6,4)):
                # extract each group
                group_traj = final_positions[idx:idx+2]
                mean_y, mean_z = np.mean(group_traj, axis=0)
                # center each group
                final_positions_y = group_traj[:, 0] - mean_y
                final_positions_z = group_traj[:, 1] - mean_z
                # append
                all_final_pos_y.extend(final_positions_y)
                all_final_pos_z.extend(final_positions_z)
...
def analyze_beam_across_x_planes(data: MainData, x_planes: Iterable[float], group_angles: Dict[int, Tuple[int, int]] = None) -> AnalysisResults:
    ...
    """
    results: AnalysisResults = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(list))))
    ...
    for fg, fg_data in data.items():
        for p_idx, trajectories in fg_data.get('local', {}).items():
            if not trajectories:
                continue
            ...
            for g_idx in range(4):
                ...
                for x_plane in x_planes:
                    ...
                    if np.any(mask):
                        ...
                        results[fg][p_idx][x_plane][key] = [mean_y, mean_z, ptp_y, ptp_z]
    return results


def find_and_store_focus(data: MainData, results: AnalysisResults, focus_axis: str = 'y', group_angles: Dict[int, Tuple[int, int]] = None):
    """
    Finds the focus for each unique (az, el) combination and stores all focus points in the data dictionary.
    ...
    Args:
        ...
        focus_axis: The axis to evaluate for focus. Can be 'y' or 'z'.
                    Defaults to 'y'.
    ...
    if focus_axis.lower() == 'y':
        axis_index = 2
        print("\nFinding focus based on minimum beam extent in Y...")
    elif focus_axis.lower() == 'z':
        axis_index = 3
        print("\nFinding focus based on minimum beam extent in Z...")
    else:
        raise ValueError("focus_axis must be 'y' or 'z'")

    min_range_results = defaultdict(lambda: defaultdict(list))  # fg -> key -> list of (ptp, x_plane, mean_y, mean_z, p_idx)

    for fg, p_data in results.items():
        for p_idx, x_plane_data in p_data.items():
            for g_idx in range(4):
                key = group_angles[g_idx] if group_angles else g_idx
                ...
                if best_plane[0] is not None:
                    ...
                    min_range_results[fg][key].append((stats[axis_index], x_plane, stats[0], stats[1], p_idx))

    # Store the focus positions back into the main data structure
    for fg, fg_min_data in min_range_results.items():
        if 'global' not in data[fg]:
            data[fg]['global'] = {}
        focus_points = {key: [(x, y, z) for _, x, y, z, _ in sorted(group_data, key=lambda x: x[4])] for key, group_data in fg_min_data.items()}
        data[fg]['global']['focus_points'] = focus_points
...
def process_data(x_range: Tuple[float, float], x_step: float, file_path: str, focus_axis: str, fly2_file: str = None):
    ...
    try:
        ...
        find_and_store_focus(data, beam_analysis_results, focus_axis=focus_axis, group_angles=group_angles)
...
def compute_vmi_parameters(field_min=200, field_max=500, num_points=20, lens_value=1.38,
    ...
    based on a range of field gradients and lens factor.

    This function iteratively adjusts the ground offset voltage until I_grid ≈ 0,
    simulating equilibrium conditions in the ion optics setup.
    ...
    Args:
        ...
        lens_value (float): Lens focusing factor (lens_VMI), typically around 1.38 (default: 1.38)
        save_to_file (bool): Whether to save results to .mat file (default: True)
        filename (str): Output .mat filename if saving (default: 'parameters.mat')

    Returns:
        dict: Dictionary containing all computed arrays:
              - field_gradient: Array of gradient values
              - Offset_to_ground: Adjusted ground offsets after convergence
              - lens_VMI: Constant lens value array
    ...
    field_gradient = np.linspace(field_min, field_max, num_points)

    # Initial guess for offset to ground (same size as field_gradient)
    Offset_to_ground = 1000 * np.ones_like(field_gradient)  # Start at 1000 V

    # Set lens_VMI as constant array using user-provided scalar
    lens_VMI = lens_value * np.ones_like(field_gradient)

    # Initialize output arrays
    I_grid = np.zeros_like(field_gradient)
    VMI2 = np.zeros_like(field_gradient)
    VMI1 = np.zeros_like(field_gradient)
    e_grid = np.zeros_like(field_gradient)
    dt_e = np.zeros_like(field_gradient)
    ...
    for idx in range(num_points):
        while True:
            ...
            I_grid[idx] = slope_factor * field_gradient[idx] + Offset_to_ground[idx]
...
def generate_particles_fly2(num_groups=4, filename='particles_tao.fly2'):
    """
        ...
        with open(filename, 'w') as fid:
            ...
            if num_groups > 0:
                ...
                for i in range(num_groups):
                    ...
                    dx = random.random() - 0.5  # Range [-0.5, 0.5]
...
def plot_fg_vs_std(processed_data, focus_criterion):
    ...
    for fg in sorted_fgs:
        data = processed_data[fg]
        if 'global' in data and std_dev_key in data['global']:
            fgs.append(fg)
            stds.append(data['global'][std_dev_key])
...
def data_viewer(data, mode='single', focus_axis='y', fg_idx=None, fly2_file=None):
    """
    ...
    Args:
        data (dict): A dictionary containing the processed data. Expected structure is
                     data[field_gradient]['global'][counts/bins_y/z, focus_xyz].
    ...
    # Compute metrics for highest peak and closest to zero
    ...
    min_std_z_fg = None
    min_std_z = float('inf')
    min_std_x_focus_fg = None
    min_std_x_focus = float('inf')

    for fg in fg_keys:
        if 'global' in data[fg]:
            global_data = data[fg]['global']
            ...
            if 'counts_z' in global_data and global_data['counts_z'].size > 0:
                ...
                if std_z < min_std_z:
                    min_std_z = std_z
                    min_std_z_fg = fg
            # Compute std of focus x positions
            focus_points = global_data.get('focus_points', {})
            if focus_points:
                x_focus = [x for points_list in focus_points.values() for x, y, z in points_list]
                if x_focus:
                    std_x_focus = np.std(x_focus)
                    if std_x_focus < min_std_x_focus:
                        min_std_x_focus = std_x_focus
                        min_std_x_focus_fg = fg

    # Prepare info strings
    info_y = f"Y: Highest peak FG {max_peak_y_fg}, Closest to zero FG {min_std_y_fg}"
    info_z = f"Z: Highest peak FG {max_peak_z_fg}, Closest to zero FG {min_std_z_fg}"
    info_x = f"X focus: Lowest std FG {min_std_x_focus_fg}"
    if mode == 'single':
        if fg_idx is None:
            ...
            fg = fg_keys[fg_idx]
        if focus_axis == 'y':
            counts, bins = data[fg]['global']['counts_y'], data[fg]['global']['bins_y']
            info = info_y
        else:
            counts, bins = data[fg]['global']['counts_z'], data[fg]['global']['bins_z']
            info = info_z
        ...
        all_data = {}
        for fg in fg_keys:
            counts_y = data[fg]['global']['counts_y']
            bins_y = data[fg]['global']['bins_y']
            if len(bins_y) > 1:
                ...
                widths_y = np.array([])
            counts_z = data[fg]['global']['counts_z']
            bins_z = data[fg]['global']['bins_z']
            if len(bins_z) > 1:
                ...
                widths_z = np.array([])
            focus_points = data[fg].get('global', {}).get('focus_points', {})
            ...
            if focus_points:
                ...
                for key, points_list in focus_points.items():
                    for x, y, z in points_list:
                        ...
                        r2s.append(y**2 + z**2)
                if len(xs_fit) > 1:
                    coeffs = np.polyfit(r2s, xs_fit, 1)
                    a, b = coeffs
                    fit_text = f'Petzval: X = {a:.3f} + {b:.3f} * r²'
        ...
        fig.subplots_adjust(bottom=0.3, hspace=0.8, wspace=0.3, top=0.85)
        ...
        # Store slider in figure to prevent garbage collection
        fig.slider = slider

        # For 3D plot interactivity
        current_scatter = None
        current_labels = []

        def on_pick(event):
            if event.artist != current_scatter:
                return
            ind = event.ind[0]
            label = current_labels[ind]
            print(f"Clicked point: {label}")

        fig.canvas.mpl_connect('pick_event', on_pick)

        def update(val):
            nonlocal current_scatter, current_labels
            fg_idx = int(val)
            fg = fg_keys[fg_idx]
            ...
            ax1.set_title(f'Histogram Y for FG {fg}')
            if d['peak_pct_y'] > 0:
            ...
            ax2.clear()
            if d['xs_y']:
                current_scatter = ax2.scatter(d['xs_y'], d['zs'], c='blue', marker='o', picker=True)
                current_labels = d['labels_y']
            ax2.set_xlabel('X position')
            ax2.set_ylabel('Z position')
            ax2.set_title(f'XZ Focus Points for FG {fg}')
            if d['fit_text']:
                ax2.text(0.05, 0.95, d['fit_text'], transform=ax2.transAxes, fontsize=9, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
            ...
            ax3.set_title(f'Histogram Z for FG {fg}')
            if d['peak_pct_z'] > 0:
            ...
            ax4.clear()
            if d['xs_z']:
                current_scatter = ax4.scatter(d['xs_z'], d['ys'], c='blue', marker='o', picker=True)
                current_labels = d['labels_z']
            ax4.set_xlabel('X position')
            ax4.set_ylabel('Y position')
            ax4.set_title(f'XY Focus Points for FG {fg}')
            if d['fit_text']:
                ax4.text(0.05, 0.95, d['fit_text'], transform=ax4.transAxes, fontsize=9, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

            suptitle_str = f"{info_y}\n{info_z}\n{info_x}\nCurrent FG {fg}\nClick on points to see Az/El"
