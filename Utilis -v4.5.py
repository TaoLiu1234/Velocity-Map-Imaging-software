...
def parse_out_file(filename: str) -> MainData:
    ...
    # List to hold all parsed simulation blocks: each is (field_gradient, lens_VMI, ke, list of (ion_n, x, y, z) points)
    all_blocks: List[Tuple[float, float, float, List[Tuple[int, float, float, float]]]] = []
    ...
    current_ke = None  # Current kinetic energy
        ...
        for line_num, line in enumerate(f, 1):
            ...
            if "Begin Next Fly'm" in line:
                ...
                if current_fg is not None and current_lens_VMI is not None and current_ke is not None and current_block_data:
                    all_blocks.append((current_fg, current_lens_VMI, current_ke, current_block_data))
    ...
    for block in all_blocks:
        if len(block) == 3:
            fg, lens_VMI, block_data = block
            ke = 10.0  # Default for old blocks
        elif len(block) == 4:
            fg, lens_VMI, ke, block_data = block
        else:
            continue
...
def process_data(x_range: Tuple[float, float], file_path: str, focus_axis: str, fly2_file: str = None, y_range: Tuple[float, float] = (-0.5, 0.5), z_range: Tuple[float, float] = (-0.5, 0.5)):
    ...
    try:
        ...
        # --- Populate global focus_points from local aligned points ---
        if group_angles:
            from collections import defaultdict
            for fg in data:
                for lvmi in data[fg]:
                    local_data = data[fg][lvmi].get('local', {})
                    for focus_axis in ['y', 'z']:
                        focus_points_dict = defaultdict(list)
                        for p_idx, p_data in local_data.items():
                            if isinstance(p_data, dict):
                                pairs = p_data.get(f'focus_point{focus_axis}', [])
                                for g_idx, pair in enumerate(pairs):
                                    if g_idx < len(group_angles):
                                        key = group_angles[g_idx]
                                        p1, p2 = pair
                                        avg_x = (p1[0] + p2[0]) / 2
                                        avg_y = (p1[1] + p2[1]) / 2
                                        avg_z = (p1[2] + p2[2]) / 2
                                        focus_points_dict[key].append((avg_x, avg_y, avg_z))
                        if 'global' not in data[fg][lvmi]:
                            data[fg][lvmi]['global'] = {}
                        data[fg][lvmi]['global'][f'focus_points_{focus_axis}'] = dict(focus_points_dict)
...
def generate_particles_fly2(num_groups=4, filename='particles_tao.fly2', x_range=(-0.5, 0.5), y_range=(-0.5, 0.5), z_range=(-0.5, 0.5), ke=15):
    """
    ...
    For each position, generates 8 trajectories with specific azimuth and elevation:
    - az=0, el=0
    - az=0, el=180
    - az=90, el=45
    - az=-90, el=45
...
def data_viewer(data, mode='single', focus_axis='y', fg_idx=None, fly2_file=None, r2_threshold=0.2, num_groups=5, y_range=(-0.5, 0.5), z_range=(-0.5, 0.5), x_range=(73.0, 166.0), electron_energy=5):
    ...
    def format_value(val):
        if val is None or val == float('inf') or np.isnan(val):
            return 'N/A'
        else:
            return f'{val:.4f}'
            ...
            for ke in data[fg][lens]:
                if 'global' in data[fg][lens][ke]:
                    ...
                    if std_min_dist_z < min_std_min_dist_z:
                        ...
                        min_std_min_dist_z_ke = ke

    info_dist = (f"Avg min distance Y/Z smallest: {format_value(min_avg_min_dist_y)} (Y) / {format_value(min_avg_min_dist_z)} (Z)\n"
                 f"  at FG {min_avg_min_dist_y_fg}/{min_avg_min_dist_z_fg}, Lens {min_avg_min_dist_y_lens}/{min_avg_min_dist_z_lens}, KE {min_avg_min_dist_y_ke}/{min_avg_min_dist_z_ke}\n"
                 f"Std min distance Y/Z smallest: {format_value(min_std_min_dist_y)} (Y) / {format_value(min_std_min_dist_z)} (Z)\n"
                 f"  at FG {min_std_min_dist_y_fg}/{min_std_min_dist_z_fg}, Lens {min_std_min_dist_y_lens}/{min_std_min_dist_z_lens}, KE {min_std_min_dist_y_ke}/{min_std_min_dist_z_ke}")
    ...
    if mode == 'single':
        ...
        for fg in fg_keys:
            for lens in data[fg]:
                d = all_data.get(fg, {}).get(lens, {})
                global_data = d.get('focus_points_y', [])
                widths_y = [item[3] for item in global_data] if global_data else []
                focus_points_z = d.get('focus_points_z', [])
                widths_z = [item[3] for item in focus_points_z] if focus_points_z else []
                if widths_y:
        ...
        info_focus = (f"Lowest Y focus width: FG {min_avg_width_y_fg} Lens {min_avg_width_y_lens}, width {format_value(min_avg_width_y)}\n"
                      f"Lowest Z focus width: FG {min_avg_width_z_fg} Lens {min_avg_width_z_lens}, width {format_value(min_avg_width_z)}")
        ...
        def update(val=None):
            ...
            if d.get('focus_points_y'):
                for item in d['focus_points_y']:
                    x, focus_y, _, w = item  # use focus_y for XZ plot
                    ax2.plot(x, focus_y, 'ro', markersize=5)  # red circle for [x, focus_y]
                    side = np.sqrt(2) * w
            ...
            ax4.set_xlim(73, 166)
            ax4.set_ylim(-45, 45)
            # Add red circles for focus centers [x, focus_z] and blue squares for radii
            if d.get('focus_points_z'):
                for item in d['focus_points_z']:
                    ...
                    ax4.plot(x, focus_z, 'ro', markersize=5)  # red circle
                    # And blue square for radii np.sqrt(2)*focus_width_z
                    side = np.sqrt(2) * w
                    #ax4.add_patch(patches.Rectangle((x - side/2, focus_z - side/2), side, side, fill=True, alpha=0.5, color='blue'))
            if d['fit_text_xy']:
